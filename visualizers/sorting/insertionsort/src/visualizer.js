...
import { insertionSortCode } from "./codeSnippets.js";
import { generateInsertionSortSteps } from "./steps.js";
...
export class InsertionSortVisualizer {
  constructor() {
    ...
    this.codeLineMap = renderCodeBlock(this.codeBlock, insertionSortCode);
    this.bindEvents();
    this.reset();
  }
  ...
  clearHighlights() {
    this.arrayElements.forEach(el => {
      el.classList.remove("current", "compare", "shift");
    });
  }
  ...
  setPointer(label, index) {
    if (!Number.isInteger(index) || index >= this.arrayElements.length) {
      // allow j = -1 badge to sit to the left of index 0
      if (index === -1 && this.arrayElements[0]) {
        const p0 = this.arrayElements[0].querySelector(".pointer-labels");
        if (!p0) return;
        const badge = document.createElement("span");
        badge.className = `pointer pointer-${label} pointer-offset-left`;
        badge.textContent = label;
        p0.appendChild(badge);
      }
      return;
    }
    if (index < 0) return;
  ...
  setKeyStatus(value, index) {
    if (!this.keyStatusEl) return;
    if (typeof value === "number" && Number.isInteger(index)) {
      this.keyStatusEl.textContent = `Key value = arr[${index}] = ${value}`;
    } else {
      this.keyStatusEl.textContent = "Key value = arr[i] = ";
    }
  }

  updateKeyStatus(step) {
    if (!this.keyStatusEl) return;
    if (step.type === "done" || step.type === "start") {
      this.setKeyStatus(null, null);
    } else if (Object.prototype.hasOwnProperty.call(step, "keyValue")) {
      this.setKeyStatus(step.keyValue, step.i);
    }
  }
  ...
  applyHighlights(step) {
    ...
    switch (step.type) {
      ...
      case "initJ":
      case "whileCheck":
        hi(step.i, "current"); hi(step.j, "compare"); break;

      case "shiftRight":
        hi(step.from, "compare"); hi(step.to, "shift"); break;

      case "decrementJ":
        hi(step.i, "current"); hi(step.j, "compare"); break;

      case "placeKey":
        hi(step.insertIndex, "shift"); break;
