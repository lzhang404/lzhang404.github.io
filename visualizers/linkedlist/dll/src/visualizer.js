...
export class DoublyLinkedListOperationsVisualizer {
  ...
  layout(list) {
    ...
    const detachedSet = new Set(list.detached ?? []);
  ...
  applyPointerHighlights(step) {
    ...
    const pointerLabels = step.pointerLabels ?? {};
    const moved = new Set();
    const previous = this.prevPointers || {};

    const pointersByNode = new Map();
    const nodeMoved = new Map();
    config.forEach(({ key }) => {
      ...
      if (nodeId != null) {
        if (!pointersByNode.has(nodeId)) pointersByNode.set(nodeId, []);
        pointersByNode.get(nodeId).push(key);
        if (moved.has(key)) nodeMoved.set(nodeId, true);
      }
    });
    ...
    config.forEach(({ key }) => {
      ...
      if (!rect) return;
      const alreadyRendered = this.gPointers.querySelector(`[data-node-id="${nodeId}"]`);
      ...
      g.setAttribute("class", `ptr ${pointerClass}${nodeMoved.get(nodeId) ? " moved" : ""}`);
      g.setAttribute("data-node-id", String(nodeId));
